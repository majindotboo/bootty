---
name: gpui-component
description: Build GPUI Kit interfaces with its gpui-component styled UI and gpui-base behavior primitives. Use for components, layout, interaction, theming, focus, and application state ownership; follow the normative Design and Coding Guides.
---

## Read This First

GPUI Kit is the framework and repository name. Its styled crate and imports remain
`gpui-component` / `gpui_component`; its behavior crate remains `gpui-base`.
Use the revision pinned in the workspace, not APIs inferred from a newer website.
The canonical repository is https://github.com/longbridge/gpui-kit.
...
- If the website is unavailable, read `website/docs/components/{name}.md` and
  the implementation from the pinned GPUI Kit checkout.

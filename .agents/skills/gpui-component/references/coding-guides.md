...
`h_flex()` centers children on the cross axis; `v_flex()` stretches them.
A row of full-height columns needs `items_stretch()` (or explicit `h_full()`
on its columns), otherwise oversized content can be centered outside its viewport.
The GPUI Kit `Scrollable` wrapper releases its own automatic minimum on the
scrolling axis, but intervening flexible containers still need `min_h_0()` or
`min_w_0()` so the actual scroll region can shrink.

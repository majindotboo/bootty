//! Ephemeral terminal facts projected for native chrome presentation.

/// A single window for native window tabs.
...
pub struct SessionView {
    ...
    /// Every pane in the session, in window order. Session modules that report per-pane facts
    /// (agents) read this so the fact survives switching the active window.
...
/// Mux state projected for native chrome.

        ...
        let cases = [
            (
                ...
                json!({ "Execute": { "command": { "SplitPane": {
                    "session_id": session, "pane_id": pane_id, "direction": direction_name,
                } } } }),
            ),
            (RemoteRmuxRequest::PaneStream { session: session.clone(), pane: pane.clone() },
             json!({ "PaneStream": { "session": session, "pane": pane } })),
            (RemoteRmuxRequest::PaneInput { session: session.clone(), pane: pane.clone() },
             json!({ "PaneInput": { "session": session, "pane": pane } })),
            ...
            (RemoteRmuxRequest::ResizeWindow { window: pane.clone(), cols, rows },
             json!({ "ResizeWindow": { "window": pane, "cols": cols, "rows": rows } })),
        ];

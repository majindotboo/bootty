...
export type TerminalSelection = WebSelection;
...
export class WebGlTerminalRenderer {
  ...
  render(frame: WebTerminalFrame, selection: TerminalSelection | null = null): void {
    ...
    const activeSelection = selection ?? frame.selection ?? null;
    if (activeSelection) {
      pushSelectionInstances(selectionInstances, frame, activeSelection);
    }
  ...
  private resize(frame: WebTerminalFrame): void {
    ...
    this.width = Math.max(1, this.canvas.clientWidth || frame.cols * frame.cellWidth);
    this.height = Math.max(1, this.canvas.clientHeight || frame.rows * frame.cellHeight);
...
class GlyphAtlas {
  ...
  private static readonly size = 4096;
  ...
  maxCellsFor(cellWidth: number, dpr: number): number {
    ...
    return Math.max(1, Math.min(24, Math.floor(usableWidth / Math.max(1, cellWidth * dpr))));
  }
  ...
  glyph(text: string, cellWidth: number, cellHeight: number, dpr: number, style: WebCell["style"]): Glyph {
    ...
    this.tileContext.fontKerning = "normal";
    const textContext = this.tileContext as CanvasRenderingContext2D & {
      fontVariantLigatures?: string;
      fontFeatureSettings?: string;
    };
    textContext.fontVariantLigatures = "common-ligatures discretionary-ligatures contextual";
    textContext.fontFeatureSettings = "\"liga\" 1, \"calt\" 1, \"dlig\" 1, \"ss01\" 1, \"ss02\" 1";
    const horizontalPadding = Math.ceil(GlyphAtlas.horizontalPaddingPx * dpr);
...
function canMergeTextRunCell(
  ...
  return isLigatureRunCodepoint(previous) && isLigatureRunCodepoint(first);
}
...
function isLigatureRunCodepoint(text: string): boolean {
  return "!<>=-:|&/#{}[]().~$%^*+?\\@ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789filw".includes(text);
}
...
function terminalFont(cellHeight: number, style: WebCell["style"]): string {
  ...
  return `${slant}${Math.floor(cellHeight * 0.78)}px "Fira Code", "Maple Mono NF", ui-monospace, SFMono-Regular, Menlo, Consolas, monospace`;
}
